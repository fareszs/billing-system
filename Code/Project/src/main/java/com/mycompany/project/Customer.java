...
public class Customer extends User{
    ...
    public void register(){
        String userData = this.getId() + "," + this.getUsername() + "," + this.meterCode + "," + this.region;
        System.out.println("Customer registered successfully: " + this.getUsername());
        String userLine = this.getId()+","+this.getPassword()+","+"customer"+","+name+","+region+","+meterCode;
        FileHandler.write("Files\\Users.txt" , userLine);
    }
    ...
    public static void payBill(String meterCode , double newReading , double oldReading){
        ...
        boolean isPaid = true;
        Bill newBill = new Bill(billId , this.getId() , value , billDate , isPaid);
    ...
    public void complaint(String meterCode , String text){
        ...
        Complaint cmp = new Complaint(complaintId, this.getId(), text);
    ...
    public void sendEmail(){
        if(this.isnew==true && isThreeMonthsPassed()){
            System.out.println("Meter of customer number : " +this.getId()+" is ready" );
        }
    }

    public void notifyCustomer(){
        if(this.isnew == false && isThreeMonthsPassed()){
            System.out.println("Customer number : "+this.getId()+" didnot pay for 3 months");
        }
    }
}
